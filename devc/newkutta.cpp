...
float pi=3.141592653589793,Ps,ei,Kv,tao,Kg,Kh,M,Cf,Bf,V10,A1,A2,AN,CN,Sd,AS,CS,AR,CR,h,AC,CC,m,C,K,sigma,Nq,Rq,Sq,Cq,a1,a2,a3,a5,a6,a7,a8,a10,a11;
...
x2 = 1.5;
x3 = (0.5421246-0.53)*30;
x4 = 0.01;
xs=0;
xr=0;
xc=0;
x5 = Nl+xs*AS+xr*2*AR+xc*2*AC;
...
//--------處理 
ut_bar=u+(ut-u)/step;
x1t_bar=x1+(x1t-x1)/step;
x3t_bar=x3+(x3t-x3)/step;
...
else
{
    if (x_bar[4]<Rl){
          ...
          x_bar[7]=Rl*AR*2;x_bar[8]=0;}
    else {
             if (x_bar[4]<Cl){
                        x_bar[6]=Sl*AS;
                        x_bar[7]=Rl*AR*2;x_bar[8]=(( tempp[0]*( Nl/AN+Sl/AS+Rl/AR)-a5*a6*y[2]- a5*a9*x_bar[3]*x_bar[3]-a5*a7*x_bar[3]*(G[0]*Nl +G[1]*Sl+G[2]*Rl))/( a5*a7*x_bar[3]*G[3]-tempp[0]/AC ))*2*CR;}             
             //else if(x_bar[4]==Cl){}
             else{
                  x_bar[6]=Sl*AS;
                  x_bar[7]=Rl*AR*2;x_bar[8]=Cl*2*AC;}
         }
}  
//---------------------------------------------------------------K2
x[0]=x_bar[0];
...
else
{
    if (y[4]<Rl){
             ...
             if (y[4]<Cl){
                        xs=Sl*AS;
                        xr=Rl*AR*2;xc=(( tempp[1]*( Nl/AN+Sl/AS+Rl/AR )-a5*a6*y[2]- a5*a9*y[3]*y[3]-a5*a7*y[3]*( G[0]*Nl +G[1]*Sl+G[2]*Rl ) )/( a5*a7*y[3]*G[3]-tempp[1]/AC ))*2*CR;}             
             //else if(y[4]==Cl){}
             else{
                  xs=Sl*AS;
                  xr=Rl*AR;xc=Cl*2*AC;}
