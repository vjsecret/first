...
//range_kutta
        for(j=0;j<=n;j++) 
        {
            ...
            k21[j]=h*f1((y1[j]+0.5*k11[j]),(y2[j]+0.5*k12[j]),(t+0.5*h));
            k22[j]=h*f2((y1[j]+0.5*k11[j]),(y2[j]+0.5*k12[j]),(t+0.5*h));
            k31[j]=h*f1((y1[j]+0.5*k21[j]),(y2[j]+0.5*k22[j]),(t+0.5*h));
            k32[j]=h*f2((y1[j]+0.5*k21[j]),(y2[j]+0.5*k22[j]),(t+0.5*h));    
            k41[j]=h*f1((y1[j]+k31[j]),(y2[j]+k32[j]),(t+h));
            k42[j]=h*f2((y1[j]+k31[j]),(y2[j]+k32[j]),(t+h));
            y1[j+1]=y1[j]+((k11[j]+2*k21[j]+2*k31[j]+k41[j])/6);
            y2[j+1]=y2[j]+((k12[j]+2*k22[j]+2*k32[j]+k42[j])/6);
            t=a+(j+1)*h;
...
//寫入A1,A2       
    fstream outFile;
    outFile.open("4kutta.txt",ios::out );
    if( outFile != NULL )    
        {   // 將資料寫入檔案中    
            for (int j=0;j<=n;j++)
            {
