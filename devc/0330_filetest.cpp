...
//range_kutta
        for(j=0;j<=n;j++) 
        {
            k11[j]=h*f1(y1[j],y2[j],t+h*j);
            k12[j]=h*f2(y1[j],y2[j],t+h*j);
            k21[j]=h*f1(y1[j]+k11[j],y2[j]+k12[j],t+h*(j+1));
            k22[j]=h*f2(y1[j]+k11[j],y2[j]+k12[j],t+h*(j+1));
...
//寫入A1,A2       
    ofstream outFile;
    outFile.open( "0329testout.txt", ios::out );
    if( !outFile.is_open())
    {
        cout<<"檔案開啟錯誤!"<< endl;
        return 1;
